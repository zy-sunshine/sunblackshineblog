...
from configs import ThemeIterator
...
from base import TemplateView, Pager
from base import requires_admin
...
from base import AdminBaseRequestHandler as BaseRequestHandler
...
class admin_sitemap(BaseRequestHandler):
    ...
    @requires_admin
    def POST(self):
        str_options= self.paramlist('str_options')
        for name in str_options:
            value=self.paramstr(name)
            setattr(self.blog,name,value)

        bool_options= self.paramlist('bool_options')
        for name in bool_options:
            value=self.parambool(name)
            setattr(self.blog,name,value)

        int_options= self.paramlist('int_options')
        for name in int_options:
            value=self.paramint(name)
            setattr(self.blog,name,value)
                
        float_options= self.paramlist('float_options')
        for name in float_options:
            value=self.paramfloat(name)
            setattr(self.blog,name,value)
...
class Upload(BaseRequestHandler):
    ...
    def POST(self):
        ...
        bits_file = self.request.FILES['upfile']
        bits = ''.join(bits_file.chunks())
...
class getMedia(BaseRequestHandler):
    def __init__(self):
        BaseRequestHandler.__init__(self)
    def GET(self, slug):
